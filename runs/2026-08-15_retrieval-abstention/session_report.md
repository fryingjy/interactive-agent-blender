...
exposes `--min-score`, and negative thresholds are rejected. A declarative 16-case regression set
covers direct and paraphrased positive contexts plus four unrelated-ticket abstention controls.
Positive cases also require at least a 1.0 top-result margin.

## Evidence

- 12/12 positive contexts retrieved the intended top skill.
- 4/4 unrelated contexts returned no skill.
- All 12 positives exceeded the frozen 1.0 top-result margin; minimum observed margin: 3.6742.
- The final suite/check counts are recorded in the question-driven research run report after the
