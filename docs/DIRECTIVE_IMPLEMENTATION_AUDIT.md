...
| Seam-authored high/low bake | official UV episode independently frame/speech verified; one longitudinal seam reproduced on a radial tube and transferred to a bent rounded-rectangle; matched no-seam controls; live source stacks; real tangent bakes and low-only GLBs | two-family controlled transfer PASS with fresh source/export verification; real-asset runtime use pending |
...
| Blender Python/BMesh | ownership, lookup tables, cleanup, triangulation maps, dissolve scope, normals, live Edit Mode destructive updates, selection flushing, edge/face/loop custom-data persistence | modeler-critical block complete; exhaustive operator signatures and API-documented TODO helpers remain open |
...
| General reference-workflow video study | seven user-supplied public videos analyzed through reproducible Gemini 3.6 audio/visual input; purpose-scoped synthesis defines typed research questions, candidate rejection, retrieval provenance, and strict stage-gating policy | one bounded reference-policy skill experimentally tested; a fresh board, human review, and equal-budget modeling transfer remain pending |
| Range-scoped reference-setup lesson transfer | Gemini `video_metadata` start/end offsets with absolute-timestamp enforcement; independent browser frames and sampled visible captions; typed FRONT/RIGHT/TOP Image Empty creation and saved-state audit; free-view and duplicated-source controls | 24–124 s episode plus Blender 5.2 transfer PASS; later whole-video timestamp drift rejected; photographic calibration and real-prop runtime use remain pending |
...
| Hard-surface shading policy | semantic bevel-weight typed operations; Blender 5.2 Bevel/SubD/Smooth-by-Angle lab; matched flat-panel comparison; twelve uniform/uneven radial and taper fixtures with unbeveled baselines, analytic side-normal metrics, MatCap evidence, live unapplied stacks, and fresh verification | implemented for tested flat and radial manufactured cases; uneven-cage evidence rejects blanket Weighted Normal, while arbitrary double curvature, SubD interaction, and reference-driven sharp intent remain open |

## Important disproved assumptions retained

- Mirror-before-SubD was not universally bad; exact curved seams passed both tested orders.
- Simple Deform `angle` and `factor` are not independent amount controls in the helper's mode.
- A clean/manifold Boolean can still contain severe tangent-groove topology defects.
...
all-quad object: the locally refined front perimeter routes into a matching circular inset, the lens grows through
