# Tutorial reproduction track

Held-out prop modeling is paused after repeated visual-accuracy and construction-graph failures.
The active learning path now requires the agent to model along with tutorials from beginner through
professional topics. Watching, transcribing, or summarizing alone never completes a lesson.

## Completion contract for every lesson

1. Verify source identity and inspect the relevant video/audio range.
2. Independently check representative frames or visible captions.
3. Build the tutorial's model or bounded chapter artifact in Blender.
4. Preserve connected topology inside each real manufactured part; separate objects only for a
   visible assembly, material, or motion boundary.
5. Inspect base and evaluated topology, modifier state, and solid/MatCap or reflective diagnostics.
6. Record at least one rejected result and correction when the render exposes a problem.
7. Apply the core technique to different geometry and record both supporting and counterevidence.
8. Promote only the rule that survived reproduction and transfer.

## Ordered ladder

| Stage | Focus | Required modeled evidence | Status |
|---|---|---|---|
| 1 | Edit Mode, continuous extrusion, radial profiles, quad openings, SubD spacing | JL Mussi energy can plus bottle-density transfer | COMPLETE WITH BOUNDED SIMPLIFICATION |
| 2 | Planar-to-curved topology and loop redirection | Tutorial model with manual edge-flow repair and a separate transfer patch | NEXT |
| 3 | Product hard surface: symmetry, bevel/crease choice, booleans, shading | Complete layered mechanical tutorial asset | PENDING |
| 4 | Advanced SubD: poles, transitions, cylinders, pinch diagnosis | Multi-problem topology sheet plus tutorial asset | PENDING |
| 5 | Retopology and deformation-aware flow | Tutorial retopo asset plus deformation test | PENDING |
| 6 | UVs, texel density, baking, and production materials | Tutorial asset carried through UV/material validation | PENDING |
| 7 | Professional asset planning, optimization, and high/low delivery | Full tutorial production asset with separate collections and live modifiers | PENDING |
| 8 | Held-out reconstruction return gate | Unseen reference asset only after two consecutive tutorial/transfer passes | BLOCKED BY CURRICULUM |

The current evidence is in `runs/2026-08-21_tutorial-jl-mussi-energy-can/`.
