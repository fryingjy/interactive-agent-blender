...
| --- | --- | --- | --- |
| P0 | `xHJzDpijPqg` / `CrCX7N_Q3XE` — robot from front/side references, parts 1–2 | **Unavailable:** direct YouTube checks on 2026-08-16 report both parts as private; no analysis was made | Obtain a user-authorized/public replacement that visibly models from matched front/side references, then review shared landmarks and front/side correction; reproduce on a non-robot reference; measure localized errors in both views |
