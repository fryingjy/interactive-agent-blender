...
def evaluate_stage_gate(stage: str, evidence: dict[str, Any], *, min_iou: float = 0.9) -> dict:
    ...
    if not missing:
        if stage == "REFERENCE_ANALYSIS":
            ...
            if evidence["view_count"] < 2: failures.append("fewer than two relevant views")
            if evidence["worst_view_iou"] < min_iou: failures.append(f"worst-view IoU below {min_iou}")
